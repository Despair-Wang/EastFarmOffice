...
class PediaController extends Controller
{
    ...
    public function callItemEditor($id = null)
    {
        ...
        if (!is_null($id)) {
            $item = PediaItem::Where('fatherId', $id)->Where('state', 1)->get();
    ...
    {
        $item = PediaItem::Where('fatherId', $id)->Where('state', 1)->first();
        ...
        // $galleries = PediaGallery::Where('itemId', $id)->Where('state', 1)->get();

        return view('pedia.preview', compact('item', 'contents', 'galleries'));
    }
    ...
    //更新分類、標籤、百科項目
    public function attrUpdate($id, $target, Request $request)
    {
        $result = false;
        $params['name'] = $request->name;
        switch ($target) {
            case 'category':
                $result = PediaCategory::Where('id', $id)->get()->update($params);
                break;
            case 'tag':
                $result = PediaTag::Where('id', $id)->get()->update($params);
                break;
            default:
                return $this->makeJson(0, null, 'UNKNOWN_CREATE_TYPE');
                break;
        ...
        foreach ($data as $id) {
            switch ($target) { //根據傳入的對象選擇更新對象
                ...
                case 'tag':
                    ...
                    break;
                case 'item':
                    $result = PediaItem::Where('id', $id)->get()->update($params);
                    break;
                default:
    ...
    public function itemCreate($oldId = null, Request $request)
    {
        try {
            ...
            $item = $result;
            if (is_null($oldId)) {
                $result = $item->update(['fatherId' => $id]);
                if (!$result) {
                    return $this->makeJson(0, $result, 'FATHER_ID_CREATE_ERROR');
                }
            }
            //建立&儲存形象圖片
            ...
            // return $this->makeJson(0, ['data' => $tags, 'type' => gettype($tags)], null);
            foreach ($tags as $tag) {
            ...
            if (!is_null($oldId)) { //有舊資料
                $oldTags = Tag_for_pedia::Where('itemId', $id)->get();
                foreach ($oldTags as $tag) {
                    $result = $tag->update(['state' => 0]);
                    if (!$result) {
                        return $this->makeJson(0, $result, 'OLD_TAGS_STATE_CHANGE_ERROR');
                    }
                }
            }
            $item = PediaItem::Where('id', $id)->get()->first();
            $tag = $item->getTags;
    ...
    {
        try {
            $params = $request->only('itemId', 'title', 'content', 'sort', 'title');
            ...
            if (count($request->allFiles()) > 0) {
                ...
                foreach ($galleries as $key => $g) {
                    ...
                    array_push($gallery, [$path . '/' . $fileName, $request[$key . 'c']]);
                }
            ...
            PediaContent::Where('itemId', $id)->Where('sort', $request->sort)->Where('state', 1)->update(['state' => 0]);
            $temp->update(['state' => 1]);
            return $this->makeJson(1, null, null);
    ...
    public function itemShow($id, $action = null)
    {
        ...
        $tags = Tag_for_pedia::Join('PediaTag', 'tag_of_pedia.tagId', '=', 'pediaTag.id')->Where('itemId', $id)->get(); //尋找項目所屬的標籤
        ...
        if ($action == 'edit') { //有傳入編輯指令時，回傳編輯畫面
            return view('pedia.editor', compact('item', 'tags', 'content', 'galleries', 'remark'));
        } else {
            return view('pedia.show', compact('item', 'tags', 'content', 'galleries', 'remark'));
        }
    }
}